...
Scheduled jobs (EB cron):
- `.ebextensions/telegram_cron.config`: `telegram_game_announcements` every minute (also ladder channel at 00:15 MSK). Log: `/var/log/telegram_cron.log`.
- `.ebextensions/difficulty_cron.config`: `refresh_daily_difficulty --limit 10` every minute. Log: `/var/log/difficulty_cron.log`. See `docs/difficulty-cache.md`.
- `telegram_daily_digest` (daily; set separately if needed).
...
**YooKassa (RUB):** monthly intro 420 → then 600. Does not use TicketRequest.
