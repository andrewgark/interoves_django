...
- **Mobile focus intent:** `activeElement` не означает открытую клавиатуру (Android
  ...
  DOM поля не меняет и программно не фокусирует. Закрытие клавиатуры (Back или
  тап по пустому месту) якорит текущую пару: снизу (`ref-role=next`) — к низу
  visualViewport, сверху — под шапку; ghost-click не должен переносить каретку
  на другое слово. Пин остаётся, пока в нём фокус или идёт набор: открытие
  клавиатуры не прячет клон и не переводит фокус в настоящее поле (иначе
  страницу уносит наверх). Ввод в пин зеркалится в настоящее поле без скролла.
  После закрытия клавиатуры скроллим пару на место, только если она ещё в
  visualViewport (уход к подсказкам не перетягиваем назад). `setRaddleFocus`
  только для playable-якоря, иначе черновик сверху открывает первое слово.
