...
- **Mobile focus intent:** `activeElement` не означает открытую клавиатуру (Android
  ...
  тап по пустому месту) не должно переносить подсветку/каретку на другое слово:
  ghost-click и кража фокуса браузером возвращают последнюю пару, без скролла и
  без смены правил стики-пина.
