...
- Отступ сверху = **реально видимая** часть `.new-nav`, не константа
  `--new-ui-scroll-margin-top`. Шапка sticky в layout viewport: при открытой
  клавиатуре страница «панится» и меню уезжает выше экрана. Тогда пин обязан
  встать в самый верх, иначе висит под пустой полосой там, где меню уже нет.
- Страница при вводе ответа **не скроллится**. `scrollIntoView` к подсказке —
  только когда подсказка и есть цель действия (нажали 💡, `revealClue: true`).
...
- Звать `scrollIntoView` из `setRaddleFocus` без разбора: он вызывается на каждый
  тап, верный ответ, live-update и перерисовку — страницу будет уносить к
  подсказке ровно в момент ввода ответа.
- Считать отступ пина от `--new-ui-scroll-margin-top`: константа не знает, что
  шапку уже не видно.
...
только зеркалит значение без `scrollIntoView`.

Вертикаль пина целиком в `positionRaddleStickyPin`, в CSS `top: 0`.
`position: fixed` считается от layout viewport, поэтому
`translateY(visualViewport.offsetTop + видимая часть шапки)`: первое слагаемое
не даёт пину уехать за экран при открытой клавиатуре, второе
(`RaddleState.visibleNavOffset`, замер `.new-nav`) поднимает пин к самому верху,
когда меню уже панингом ушло из видимой области.
