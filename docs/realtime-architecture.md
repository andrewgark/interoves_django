...
| Daily solve timer | Supported section daily games use one server-side `DailySolveTiming` row per canonical actor; Team pages resolve the actor from the authenticated member's selected play mode and share the Team row through the existing HTTP timer endpoint | Team pause/resume is shared state, but a different teammate's open timer UI is not pushed a live timing snapshot; the next timer request/page reconciliation observes it |
...
| Delivery observability | Structured `realtime_sync` logs record publish, WebSocket delivery, and reconciliation metadata (scope, actor/recipient, task, reason, sequence) without payload HTML | Historical client-side receipt is unavailable until this instrumentation is deployed |
