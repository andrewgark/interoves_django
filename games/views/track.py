...
def track_channel_name(game_id, team_id):
    return f'game.track.{game_id}.{team_id}'

class TrackGame(JsonWebsocketConsumer):
    def connect(self):
        self.user_id = self.scope['user'].id
        self.team_id = self.scope['user'].profile.team_on_id
        self.game_id = self.scope['url_route']['kwargs']['game_id']
        self.group = track_channel_name(self.game_id, self.team_id)
