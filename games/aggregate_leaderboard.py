...
def _build_legacy_aggregate_page(request, game, *, window_context=None):
    ...
    if actors:
        ...
        from games.daily_section import is_scheduled_game
        ...
        if is_scheduled_game(game.id):
            ...
            task_groups_by_id = {
                column.link.task_group_id: column.link.task_group
                for column in columns
            }
            for timing in timing_rows:
                ...
                seconds = canonical_elapsed_seconds(
                    game=game,
                    task_group=task_groups_by_id.get(timing.task_group_id),
                    timing_row=timing,
                )
